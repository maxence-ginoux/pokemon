...
class Game:
    ...
    def start_game(self):
        # Chargez les données du fichier JSON dans le pokedex
        if not self.pokedex.load_pokedex():
            print("Erreur : échec du chargement des données Pokémon.")
            return
        ...
        # Affichez le message du gagnant
        print(self.winner_message)
    ...
    def play_game(self):
        ...
        while running:
            ...
            # Implémentez la logique du jeu en utilisant la classe Combat
            if self.combat.is_battle_over(self.player_pokemon, self.opponent_pokemon):
            ...
            # Gérez les entrées utilisateur (remplacez par votre propre logique de gestion des entrées)
            keys = pygame.key.get_pressed()
            if keys[K_SPACE]:
                # Par exemple, attaquer lorsque la touche espace est enfoncée
                self.combat.attack(self.player_pokemon, self.opponent_pokemon)

            # Mettez à jour l'affichage
            self.screen.fill(BLANC)
            self.draw_pokemon(self.player_pokemon, 50, 50)
            self.draw_pokemon(self.opponent_pokemon, 600, 50)
            # Dessinez des barres de santé
            self.draw_health_bar(self.player_pokemon, 50, 30)
            self.draw_health_bar(self.opponent_pokemon, 600, 30)
            # Dessinez d'autres éléments du jeu
            # ...
    ...
    def draw_pokemon(self, pokemon, x, y):
        if pokemon:
            text = self.font.render(pokemon.nom, True, BLANC)
            self.screen.blit(text, (x, y))

    def draw_health_bar(self, pokemon, x, y):
        if pokemon:
            # Dessine le fond de la barre de santé
            pygame.draw.rect(self.screen, ROUGE, (x, y, 100, 20))

            # Évitez une division par zéro potentielle
            if pokemon.point_de_vie > 0:
                # Calculez la largeur de la barre de santé en fonction de la santé actuelle
                health_percentage = pokemon.point_de_vie / pokemon.point_de_vie
                bar_width = int(health_percentage * 100)

                # Dessine la barre de santé
                pygame.draw.rect(self.screen, VERT, (x, y, bar_width, 20))

            # Affichez la santé actuelle sous forme de texte
            health_text = self.font.render(f"PV : {pokemon.point_de_vie}/{pokemon.point_de_vie}", True, BLANC)
            self.screen.blit(health_text, (x + 110, y))
...
if __name__ == "__main__":
    ...
    # Instancier et exécuter la classe PokemonGame
game = Game()
game.start_game()
